...
Pdg_min=0.05*Pn_DG; % LR_DG
  ...
  for t=1:NT
      ...
      [Pch_max,Pdch_max]=Battery_Model(Nbat,Eb(t),alfa_battery,c,k,Imax,Vnom,ef_bat); % kW
      
    %%
    if P_RE(t)>=(Eload(t)/n_I) && (Eload(t)<=Pinv_max) %  if PV+Pwt greater than load  (battery should charge)
