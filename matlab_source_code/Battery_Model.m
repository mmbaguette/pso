...
function [Pch_max,Pdch_max]=Battery_Model(Nbat,Eb,alfa_battery,c,k,Imax,Vnom,ef_bat)
...
Qmax=1*Nbat;  % is the total capacity of the storage bank [kWh]
...
Pdch_max=( k*Q1*exp(-k*dt) +Q*k*c*(1-exp(-k*dt)) )/(1-exp(-k*dt)+c*(k*dt-1+exp(-k*dt)) ) *sqrt(ef_bat);
