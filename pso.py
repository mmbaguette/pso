...
def energy_management(
    Ppv, Pwt, Eload, Cn_b, Nbat, Pn_DG, NT, Cn_I, cc_gen, Cbw
):
...
def fitness(X): # what is x? assume it is an array with 5 indices - might be better to explicitly define params
    ...
    global b, C_fuel, R_DG, TL_DG, MO_DG
    ...
    # TODO: double check the exponent operator
    ...
    # Total M&O Cost ($/year)
    # TODO: raise to power of a vector column
    # MO_Cost=(MO_PV*Pn_PV + MO_WT*Pn_WT + MO_DG*sum(Pn_DG[Pn_DG>0])+ MO_B*Cn_B+ MO_I*Cn_I +MO_CH) / (1+ir).^(1:25) ;

    # DG fuel Cost
    # TODO: power to vector column
    ...
    # TODO: replace all NaN values with 0?
    RE.fillna(0)

    Z=LCOE+EM*LEM+10*(LPSP[LPSP>LPSP_max])+10*(RE[RE<RE_min])+100*(T_Cost[I_Cost>Budget])+100*max(0, LPSP-LPSP_max)+100*max(0, RE_min-RE)+100*max(0, I_Cost-Budget)
